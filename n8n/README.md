...
7 个图片工作流在模型出图后调用同一 Docker 网络中的 `ews-image-sidecar`。sidecar 下载模型临时 URL，使用 `sharp/libvips` 转为 quality 88 JPEG；输出超过 1.9MB 时才使用 Bilinear 等比缩小。随后它复用推送体中的 `callback_secret` 请求 `POST /api/internal/r2-upload-ticket`，使用 5 分钟有效、仅允许写入当前计划对象的 presigned PUT URL 直传 R2。成功回调只传 `r2_key`、`sha256`、`size_bytes` 和定位字段，不传 Base64 或临时图片 URL。
...
sidecar 通过 `D:\DockerApps\n8n\compose.yaml` 运行，不映射宿主机端口；n8n 仅通过 `http://ews-image-sidecar:3000` 内网访问。镜像更新命令：

```powershell
docker build -t ews-image-sidecar:2026-07-17 n8n/image-sidecar
docker compose -f D:\DockerApps\n8n\compose.yaml up -d image-sidecar
```
