...
// ==================== Shopee 模块 ====================
async function shopeeCreateProduct(env, p) { await env.DB.prepare("INSERT INTO ews_shopee_products (id, task_id, category_id, name, description, parent_sku, cover_image, images, weight_kg, length_cm, width_cm, height_cm, gtin, brand_id, hs_code, tax_code, origin_country, variation_name1, variation_name2, max_purchase_qty, size_chart_template_id, size_chart_image, pre_order_dts, shipping_channels, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', datetime('now'), datetime('now'))").bind(p.id, p.task_id, p.category_id || '', p.name, p.description || '', p.parent_sku || '', p.cover_image || '', p.images || '[]', p.weight_kg || 0, p.length_cm ?? null, p.width_cm ?? null, p.height_cm ?? null, p.gtin || '', p.brand_id || '', p.hs_code || '', p.tax_code || '', p.origin_country || '', p.variation_name1 || '', p.variation_name2 || '', p.max_purchase_qty ?? null, p.size_chart_template_id || '', p.size_chart_image || '', p.pre_order_dts ?? null, p.shipping_channels || '[]').run(); }
async function shopeeGetProduct(env, pid) { var p = await getOne(env, "SELECT * FROM ews_shopee_products WHERE id = ?", [pid]); if (!p) return null; p.variations = (await query(env, "SELECT * FROM ews_shopee_variations WHERE product_id = ?", [pid])).results; p.sub_tasks = (await query(env, "SELECT * FROM ews_shopee_sub_tasks WHERE parent_task_id = ? ORDER BY set_index", [pid])).results; p.images_rec = (await query(env, "SELECT * FROM ews_shopee_task_images WHERE parent_task_id = ? ORDER BY set_index,image_type,position", [pid])).results; return p; }
...
async function shopeeCreateExpectedImages(env, tid, sid, si, mic, dic) {
  var types = []; types.push({ type: 'main', pos: 1 });
  for (let p = 2; p <= mic; p++) types.push({ type: 'sub', pos: p });
  for (let p = 1; p <= dic; p++) types.push({ type: 'detail', pos: p });
